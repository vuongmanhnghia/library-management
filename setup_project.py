...
check_installed(["python", "--version"], "Python")
...
frontend_process = subprocess.Popen(
    f'start cmd /k "cd /d {FRONTEND_DIR} && npm install && npm start"',
    shell=True
)

# Đợi 1 giây để tránh xung đột giữa hai terminal
time.sleep(1)
...
backend_process = subprocess.Popen(
    f'start cmd /k "cd /d {BACKEND_DIR} && call venv\\Scripts\\activate && pip install -r requirements.txt && pip uninstall -y motor pymongo && pip install -y motor pymongo && uvicorn app.main:app --reload"',
    shell=True
)

print("✅ Hai terminal đã mở. Kiểm tra cửa sổ command prompt!")
